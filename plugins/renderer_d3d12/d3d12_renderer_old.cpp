...
namespace {
    ...
    BindGroupHandle create_bind_group(const BindGroupDesc* d) {
        ...
        for (uint32_t i=0; i<d->entry_count; ++i) {
            ...
            if (e.type == BindGroupEntryType::Texture && e.texture) bg.texture = e.texture;
            if (e.type == BindGroupEntryType::Sampler && e.sampler) bg.sampler = e.sampler;
            if (e.type == BindGroupEntryType::UniformBuffer && e.buffer) {
